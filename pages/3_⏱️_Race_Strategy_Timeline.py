...
def create_strategy_plot(session: fastf1.core.Session) -> tuple[go.Figure, list[str]]:
    ...
    driver_totals = (
        stints.groupby("Driver")["StintLength"].sum().sort_values(ascending=False)
    )
    ...
    fig.update_layout(
        ...
        yaxis_title="Driver",
        yaxis=dict(
            categoryarray=fixed_driver_order, categoryorder="array", fixedrange=False
        ),
        xaxis=dict(range=[0, max_race_length * 1.05], fixedrange=False),
        height=800,
        showlegend=True,
        hovermode="closest",
        legend=dict(
